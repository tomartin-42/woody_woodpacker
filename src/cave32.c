...
static int align_up(uint64_t value, uint32_t alignment, uint64_t *result) {
  uint32_t mask;
  ...
  *result = (value + mask) & ~(uint64_t)mask;
  return (1);
}
...
static int load_data(size_t origin_len, t_elf_info *elf_info,
  ...
  cave_info->new_phnum = ((const Elf32_Ehdr *)elf_info->ehdr)->e_phnum + 1;
  ...
  cave_info->phdr_size = cave_info->new_phnum * sizeof(Elf32_Phdr);
  ...
  cave_info->payload_size =
      (uint64_t)((uintptr_t)payload32_end - (uintptr_t)payload32_start);

  // Comprueba que el segmento completo cabe en offsets y direcciones ELF32.
  if (cave_info->payload_offset < cave_info->injection_offset ||
      cave_info->payload_size > UINT32_MAX)
    return (0);

  relative_offset =
      cave_info->payload_offset - cave_info->injection_offset;
  if (relative_offset > UINT32_MAX - cave_info->payload_size)
    return (0);
  segment_size = relative_offset + cave_info->payload_size;

  if (cave_info->new_segment_vaddr > UINT32_MAX - segment_size ||
      cave_info->injection_offset > UINT32_MAX - segment_size)
    return (0);

  // Convierte el offset del payload en su dirección virtual dentro del segmento
  cave_info->payload_vaddr = cave_info->new_segment_vaddr + relative_offset;

  return (1);
}

static Elf32_Phdr *generate_new_phdr(t_elf_info *elf_info,
                                     t_cave_info *cave_info) {
  const Elf32_Ehdr *ehdr;
  Elf32_Phdr *new_phdrs;
  Elf32_Phdr *new_segment;
  unsigned int found;
  ...
  ft_memcpy(new_phdrs, elf_info->phdrs, ehdr->e_phnum * sizeof(Elf32_Phdr));
  ...
  new_segment->p_offset = (Elf32_Off)cave_info->injection_offset;
  new_segment->p_vaddr = (Elf32_Addr)cave_info->new_segment_vaddr;
  new_segment->p_paddr = (Elf32_Addr)cave_info->new_segment_vaddr;
  ...
  new_segment->p_filesz =
      (Elf32_Word)(cave_info->payload_offset + cave_info->payload_size -
  ...
  for (size_t i = 0; i < ehdr->e_phnum; i++) {
    if (new_phdrs[i].p_type == PT_PHDR) {
      found++;
      new_phdrs[i].p_offset = (Elf32_Off)cave_info->phdr_offset;
      ...
      new_phdrs[i].p_vaddr =
          (Elf32_Addr)(cave_info->new_segment_vaddr +
                       (cave_info->phdr_offset -
                        cave_info->injection_offset));

      // PT_PHDR describe exactamente la nueva tabla en memoria y en archivo
      new_phdrs[i].p_paddr = new_phdrs[i].p_vaddr;
      new_phdrs[i].p_filesz = (Elf32_Word)cave_info->phdr_size;
      new_phdrs[i].p_memsz = (Elf32_Word)cave_info->phdr_size;
...
static unsigned char *generate_payload(t_elf_info *elf_info,
  ...
  ft_memcpy(payload, payload32_start, payload_size);
  ...
  offset =
      (size_t)((uintptr_t)payload32_text_delta - (uintptr_t)payload32_start);

  // Parchea la distancia relativa desde el payload hasta la sección .text.
  delta = (uint32_t)elf_info->text_vaddr - (uint32_t)cave_info->payload_vaddr;
  ft_memcpy(payload + offset, &delta, sizeof(delta));

  // Parchea la distancia relativa hasta el entry point original.
  offset =
      (size_t)((uintptr_t)payload32_entry_delta - (uintptr_t)payload32_start);
  delta = (uint32_t)elf_info->old_entry - (uint32_t)cave_info->payload_vaddr;
  ft_memcpy(payload + offset, &delta, sizeof(delta));

  // Parchea el tamaño de .text usando el dword reservado en el payload.
  offset =
      (size_t)((uintptr_t)payload32_text_size - (uintptr_t)payload32_start);
  ...
  offset = (size_t)((uintptr_t)payload32_key_size - (uintptr_t)payload32_start);
  patched_size = key_size;
  ft_memcpy(payload + offset, &patched_size, sizeof(patched_size));

  // Sustituye los bytes reservados para la clave dentro de la copia.
  offset = (size_t)((uintptr_t)payload32_key - (uintptr_t)payload32_start);
...
int generate_cave32(unsigned char *origin_file, size_t origin_len,
  ...
  // Patch ehdr original
  ehdr->e_phoff = (Elf32_Off)cave_info->phdr_offset;
  ehdr->e_phnum = (Elf32_Half)cave_info->new_phnum;
  ehdr->e_entry = (Elf32_Addr)cave_info->payload_vaddr;
