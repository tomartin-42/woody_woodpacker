...
void main_get_elf32_data(t_woody *woody, void *origin_file,
                         ssize_t origin_len) {
...
void get_elf32_sheader(t_woody *woody, void *origin_file) {
  ...
  woody->s_header =
      (Elf64_Shdr *)malloc((sizeof(Elf32_Shdr) * (woody->header->e_shnum)));
...
unsigned int calculate_padding(t_woody *woody, ssize_t origin_len) {
  Elf32_Addr highest_paddr = get_max_paddr(woody);
